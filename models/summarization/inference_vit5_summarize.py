...
def main():
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Bọc text_vi dạng gần giống dữ liệu train (mặc định chỉ thêm prefix summarize: )",
    )
    args = p.parse_args()
    ...
    if args.lang == "vi":
        ...
        text_in = wrap_vi_like_training(text_vi) if args.wrap_vi_like_training else text_vi
        out = s.summarize(
            text_in,
        ...
        vi_in = wrap_vi_like_training(text_vi) if args.wrap_vi_like_training else text_vi
